...
let getTheLink typingRuleForTerm typingRuleForNested = 
	match typingRuleForTerm with Rule(name,premises1,conclusion1) ->
	match typingRuleForNested with Rule(name,premises2,conclusion2) -> 
		match getTermInOutput (List.hd (getPremises typingRuleForTerm)) with Constructor(name1, arguments1) -> 
			match getTermInOutput (getConclusion typingRuleForNested) with Constructor(name2, arguments2) -> List.combine arguments1 arguments2
...
let rec substitutionsTerm associations applicationsInConclusion term = match term with
         ...
         | Application(term1,term2) ->  
                  ...
                  let var2lookedUp = (try (List.assoc var1Substituted applicationsInConclusion) with Not_found -> raise (Failure (toString var1Substituted))) in
...
let flattenTheDoubleApplication signatureTerms typingRules rule = match rule with Rule(name,premises,conclusion) ->
         ...
         let applicationsInConclusion = retrieveApplications (getTermInOutput (getConclusion rule)) in
		 ...
		 let linkingTheTwoRules = getTheLink typingRuleForTerm typingRuleForNested
         let associationsForTerm = linkingTheTwoRules @ (List.combine (getArgumentsOfConstructor (getTermInInput (getConclusion typingRuleForTerm))) (getArgumentsOfConstructor term)) in
