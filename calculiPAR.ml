...
let abs_decl = [DeclTrm("abs", Constr "arrow", Contextual [], [Abstraction("term", "term") ; Simple("typ") ])]
let app_decl = [DeclTrm("app", Destr "arrow", Contextual [(1,[]) ; (2,[1])], [Simple("term"); Simple("term")])]
...
let add_decl = [DeclTrm("add", Destr "int", Contextual [(1,[]) ; (2,[1])], [Simple("term"); Simple("term")])]
...
let stlc_par = TypeSystem(	
					arrow_decl, 
					abs_decl @ app_decl,
					abs @ app @ beta)

(* The real reduction rule for addition is the following. Though is not supported. 
		Rule("addSucc",
				 [Formula("step", [Constructor( "add", [Var("E1") ; Var("E2")])], [Var("Z")])], 
				Formula("step",
					[Constructor( "add", [Constructor( "succ", [Var("E1")]) ; Var("E2")])],
    				[Constructor( "succ", [Var("Z")])]  )  ) 			
...
let addSemantics = 
		...
		Rule("addSucc",
				...
				Formula("step",
					[Constructor( "add", [Constructor( "succ", [Var("E1")]) ; Var("E2")])],
    				[Constructor( "succ", [Var("E1")])]  )  ) 			
		] in 
			TypeSystem( 
		   			arrow_decl @ int_decl,
					abs_decl @ zero_decl @ succ_decl @ app_decl @ add_decl,
					abs @ zero @ succ @  app @ add @ beta @ addSemantics)
